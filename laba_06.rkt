...
(define (my-member a lst) ;main define
	(if (empty? lst) #f
...
; Task 1
(define (ulength lst) ;main define
...
; Task 2
(define (uquantity lst) ;main define
...
; Task 4
(define (mprimes lst1 lst2) ;main define
	(define (gcd a b)
		(if (= b 0) a ;first part
			(gcd b (remainder a b))
		)
	)
	(define (iter l1 l2 out) ;second part
	...
	(iter lst1 lst2 '()) ;output
)

; Task 5
(define (sublists lst) ;main define
	(define (iter rev out)
		(if (empty? rev) out
			( iter (cdr rev) (cons (reverse rev) out) ) ;look at the unit
		)
	)
	(iter (reverse lst) '()) ;output
)
 ;оно точно с багом, не может быть так просто
